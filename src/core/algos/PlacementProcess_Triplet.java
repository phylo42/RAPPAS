...
import core.hash.Triplet;
import core.hash.Triplet_16_32_32_bit;
import etc.Infos;
import inputs.Fasta;
 ...
 * @author ben
 */
public class PlacementProcess_Triplet {
     ...
     */
//    public PlacementProcess_Triplet(SessionNext_V2 session, Float nsBound, int queryLimit) {
//        this.session=session;
//        this.nsBound=nsBound;
//        this.queryLimit=queryLimit;
//    }
    public PlacementProcess_Triplet(SessionNext_Triplet session, Float nsBound, int queryLimit) {
        this.session=session;
        this.nsBound=nsBound;
        this.queryLimit=queryLimit;
    }
...
//            Infos.println("Launching scoring on candidate nodes...");
            ...
            while ((qw=sk.getNextByteWord())!=null) {
                ...
                List<Triplet> allTriplets = session.hash.getTriplets(qw);
...
//                long startT1Time=System.currentTimeMillis();
                ...
                List<Triplet> allTriplets =null;
                if (session.states instanceof DNAStates) {
//                    allTriplets = session.hash.getTriplets(session.states.compressMer(qw));
//                } else {
                    allTriplets = session.hash.getTriplets(qw);
                }
...
//                allTriplets.stream().forEach((Char2FloatMap.Entry entry) -> {
...
//                    
//                });
                allTriplets.stream().forEach((Triplet entry) -> {
                    ...
                    nodeOccurences[nodeId]+=1;
                    //score associated to originalNode x for current read
                    nodeScores[nodeId]+=entry.getPPStar();
...
//                       graphDataForTopTuples[2][queryKmerCount*xSize+topPosition]= new Double((float)allTriplets.toArray()[0]);                        
//                    }
//
