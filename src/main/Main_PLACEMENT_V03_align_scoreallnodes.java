 ...
 * @author ben
 */
public class Main_PLACEMENT_V03_align_scoreallnodes {
    ...
    public static int Main_PLACEMENT_V03_align_scoreallnodes(File q, File db, File workDir) {
        ...
        try {
            ...
            if (s instanceof DNAStates)
                analysisType=Main_PLACEMENT_V03_align_scoreallnodes.TYPE_DNA;
            else if (s instanceof AAStates)
                analysisType=Main_PLACEMENT_V03_align_scoreallnodes.TYPE_PROTEIN;
            ...
            HashMap<Fasta,HashMap<Integer,ArrayList<ArrayList<Object>>>> placementsThroughDiagsums=new HashMap();
            HashMap<Fasta,LinkedList<Score>> placementCompleteScore=new HashMap<>(); //fasta-> score=(score;PP*)
            ...
            while ((fasta=fp.nextSequenceAsFastaObject())!=null) {
                ...
                if (!placementsThroughDiagsums.containsKey(fasta))
                    placementsThroughDiagsums.put(fasta, new HashMap<>());
...
//                }
                ...
                int[] nodeIdToDiagsumListIndex = new int[tree.getNodeCount()];  //index[score]=preplacementDiagSumIndex
                int[] diagsumListIndexToNodeId =new int[encounteredCount];
                ...
                ArrayList<DiagSum> diagsumsPerNode = new ArrayList<>(encounteredCount);
                int index=0;
                for (int i=0;i<nodesEncountered.length;i++) {
                    if (nodesEncountered[i]==false) {continue;}
                    diagsumsPerNode.add(new DiagSum(queryLength, align.getLength(), minOverlap, k, sk.getStep()));
                    diagsumListIndexToNodeId[index]=i;
                    nodeIdToDiagsumListIndex[i]=index;
                    diagsumsPerNode.get(index).init(thresholdAsLog); 
                    index++;
                }
                    ...
                    for (int refPos:allRefPos) {
                        ...
                        for (int i = 0; i < allTuples.size(); i++) {
                            ...
                            if (bestPPStarsDiagsum.getEffectiveWordCount(diagSumPos)>=((20-k+1)/sk.getStep())) {
                                //condition of nodes encountered at alignment
                                if (nodesEncountered[tuple.getNodeId()]) {
                                    diagsumsPerNode.get(nodeIdToDiagsumListIndex[tuple.getNodeId()]).sum(diagSumPos, -thresholdAsLog+tuple.getPPStar());
                                    nodesAttained[tuple.getNodeId()]=true;
                                }
                            }
                ...
                Infos.println("Peeks surviving tests (diagsum approach): "+countSurviving);
                //check how many nodes have been fully tested
                int countNodesAttained=0;
                for (int i = 0; i < nodesAttained.length; i++) {
                    if(nodesAttained[i]==true)
                        countNodesAttained++;
                }
                Infos.println("Nodes attained (diagsum approach): "+countNodesAttained);    
                ...
                for (int i = 0; i < positionSurvivingTest.length; i++) {
                    if(positionSurvivingTest[i]==true) {
                        ...
                        //Infos.println("For peek at diagSumPos="+i+" score="+diagsumListIndexToNodeId[peekIndex]+" appears to be the best placement.");
                        //Infos.println("Node details: "+tree.getById(diagsumListIndexToNodeId[peekIndex]));
                        placementsThroughDiagsums.get(fasta).put(diagsumListIndexToNodeId[peekIndex], new ArrayList<>());
                        ArrayList<Object> vals=new ArrayList<>();
                        vals.add(diagsumsPerNode.get(0).getPositionInReference(i)); //position
                        vals.add(peekValue); //PP*
                        placementsThroughDiagsums.get(fasta).get(diagsumListIndexToNodeId[peekIndex]).add(vals);
                    }
                }                
                ...
                placementCompleteScore.put(fasta, new LinkedList<>());
                for (int i = 0; i < completeScore.length; i++) {
                    if (completeScore[i]!=baseline) {
                        placementCompleteScore.get(fasta).add(new Score(i, completeScore[i]));
                    }
                }
                Collections.sort(placementCompleteScore.get(fasta));
...
//            
            ...
            ////////////////////////////////////////////////////////////////////
            Infos.println("Generating placement (completeScores) in .tsv format...");
            ...
            for (Iterator<Fasta> iterator = placementCompleteScore.keySet().iterator(); iterator.hasNext();) {
            ...
            ////////////////////////////////////////////////////////////////////
            //DIAGSUMS OUTPUT THE PLACEMENTS IN TSV FORMAT (diagsum based)
            ////////////////////////////////////////////////////////////////////
            Infos.println("Generating placement (diagsum-based) in .tsv format...");
            CSVWriter fwPlacement2=new CSVWriter(new FileWriter(new File(logPath+"placements_per_diagsum.tsv")), '\t');
            String[] header2=new String[5];
            header2[0]="Query";
            header2[1]="NodeId";
            header2[2]="ExtNodeId";
            header2[3]="Align_start";
            header2[4]="PP*";
            fwPlacement2.writeNext(header2);
            String[] data2=new String[5];
            for (Iterator<Fasta> iterator = placementsThroughDiagsums.keySet().iterator(); iterator.hasNext();) {
                Fasta fastaMaster = iterator.next();
                for (Iterator<Fasta> iteratorSlave = identicalQueries.get(fastaMaster).iterator(); iteratorSlave.hasNext();) {
                    Fasta fastaSlave = iteratorSlave.next();
                    data2[0]=fastaSlave.getHeader();
                    for (Iterator<Integer> iterator1 = placementsThroughDiagsums.get(fastaSlave).keySet().iterator(); iterator1.hasNext();) {
                        ...
                        for (Iterator<ArrayList<Object>> iterator2 = placementsThroughDiagsums.get(fastaSlave).get(nextNode).iterator(); iterator2.hasNext();) {
                            ...
                            data2[4]=String.valueOf((Float)nextPosition.get(1));
                            fwPlacement2.writeNext(data2);
                        }
                    }
                }
            }
            ...
            ////////////////////////////////////////////////////////////////////
            //OUTPUT THE PLACEMENTS IN JPLACE (JSON) FORMAT (completeScore)
            ////////////////////////////////////////////////////////////////////
            Infos.println("Generating placement (completeScore) output in .jplace format...");
            ...
            for (Iterator<Fasta> iterator = identicalQueries.keySet().iterator(); iterator.hasNext();) {
                ...
                //map(query)=(map(score)=positions_in_align)
                for (Iterator<Score> iterator1 = placementCompleteScore.get(currentFasta).iterator(); iterator1.hasNext();) {
            ...
            ////////////////////////////////////////////////////////////////////
            //OUTPUT THE PLACEMENTS IN JPLACE (JSON) FORMAT (diagsum based)
            ////////////////////////////////////////////////////////////////////
            Infos.println("Generating placement (diagsum-based) output in .jplace format...");
            //with library json.simple
            level=0;
            top=new JSONObject(); 
            ...
            for (Iterator<Fasta> iterator = identicalQueries.keySet().iterator(); iterator.hasNext();) {
                ...
                JSONArray allPlaces=new JSONArray();
                //map(query)=(map(score)=positions_in_align)
                for (Iterator<Integer> iterator1 = placementsThroughDiagsums.get(currentFasta).keySet().iterator(); iterator1.hasNext();) {
                    //this is the node to which the read was placed
                    Integer nodeId = iterator1.next();
                    for (Iterator<ArrayList<Object>> iterator2 = placementsThroughDiagsums.get(currentFasta).get(nodeId).iterator(); iterator2.hasNext();) {
            ...
            fwJSON =new FileWriter(new File(logPath+"placements2.jplace"));
            fwJSON.append(out);
            fwJSON.close();
                    
            long endOutputsTime=System.currentTimeMillis();
            System.out.println("Generating output files took: "+(endOutputsTime-startOutputsTime)+" ms");
            ...
            Logger.getLogger(Main_PLACEMENT_V03_align_scoreallnodes.class.getName()).log(Level.SEVERE, null, ex);
            return -1;
        }
