 ...
 * @author ben
 */
public class Main_prunning_tests {
    ...
    public static void main(String[] args) {
        ...
        //TEST PRUNING
        //baseDirs
        ArrayList<File> prunedDirectories=new ArrayList<>();
        String baseDir="/media/ben/STOCK/DATA/ancestral_reconstruct_tests/paml/alpha_RNApol/model_GTRnuc/pruning_versions/";
        prunedDirectories.add(new File(baseDir+"prune_leaf_AB907634/"));
        prunedDirectories.add(new File(baseDir+"prune_leaf_AF516906/"));            
        prunedDirectories.add(new File(baseDir+"prune_leaf_KJ473819/"));
        prunedDirectories.add(new File(baseDir+"prune_leaf_KF843858/"));
        prunedDirectories.add(new File(baseDir+"prune_clade_33/"));
        prunedDirectories.add(new File(baseDir+"prune_clade_47/"));
        prunedDirectories.add(new File(baseDir+"prune_clade_51/"));
        //build complete file lists from the base
        ArrayList<File> prunedAlign=new ArrayList<>();
        ArrayList<File> prunedTree=new ArrayList<>();
        ArrayList<File> prunedStats=new ArrayList<>();
        ArrayList<File> reads=new ArrayList<>();
        for (File file : prunedDirectories) {
            System.out.println(file.getAbsolutePath()+File.separator+file.getName().replace("prune", "prunealign")+".mfa.1");
            prunedAlign.add(new File(file.getAbsolutePath()+File.separator+file.getName().replace("prune", "prunealign")+".mfa.1"));
            System.out.println(file.getAbsolutePath()+File.separator+file.getName().replace("prune", "prunetree")+".tree.1.2");
            prunedTree.add(new File(file.getAbsolutePath()+File.separator+file.getName().replace("prune", "prunetree")+".tree.1.2"));
            System.out.println(new File(file.getAbsolutePath()+File.separator+"rst"));
            prunedStats.add(new File(file.getAbsolutePath()+File.separator+"rst"));
        ...
        for (int i = 0; i < prunedDirectories.size(); i++) {
            
            //debug
            //i=1;
            
            alignmentFile=prunedAlign.get(i);
            treeFile=prunedTree.get(i);
            probaFile=prunedStats.get(i);
            ...
            c.store(new File(prunedDirectories.get(i)+File.separator+"PAML_session_params_"+k+"_"+k+"_1e-6_1e-6_coronavirus_alpha"));
            im=null;
            c=null;
            System.gc();
        ...
        InputManager im=new InputManager(InputManager.SOURCE_PAML, alignmentFile, treeFile, probaFile, s);
        c.associateInputs(im);
        c.generateColmerSetData(true,ColmerSet.SAMPLING_LINEAR);
        ...
        */
        ...
        final Session session=Session.load(new File(prunedDirectories.get(testIndex ).getAbsolutePath()+"/PAML_session_params_"+k+"_"+k+"_1e-6_1e-6_coronavirus_alpha"));
...
//                            }
                ...
                Logger.getLogger(Main_prunning_tests.class.getName()).log(Level.SEVERE, null, ex);
            }
